...
function parseTaskFile(full) {
  ...
  return {
    ...
    scheduled: fm.scheduled || '', order: fm.order || '', completed_at: fm.completed_at || '',
    notes, plan,
...
app.post('/tasks', (req, res) => {
  ...
  const priority = /^[123]$/.test(clean(b.priority)) ? clean(b.priority) : '';
...
const UPDATABLE_FIELDS = ['area', 'type', 'person', 'duration', 'priority', 'project', 'scheduled', 'order'];
...
function listTranscriptSessions() {
  ...
  for (const f of files) {
    ...
    out[id] = { updatedAt: mt, preview: firstUserText(full), lastMessage: lastUserText(full) };
  }
  return out;
}
...
app.get('/history/:sessionId', (req, res) => {
  ...
  if (data && Array.isArray(data.messages) && data.messages.length) {
    const healed = healEmptyAssistantText(id, data.messages);
...
let authProc = null, authBuf = '', authUrl = '', authDone = false, authResult = null, authToken = '';
...
function killAuth() { if (authProc) { try { authProc.kill('SIGKILL'); } catch {} } authProc = null; }
...
app.get('/usage', (req, res) => {
  ...
  rateLimits.email = readAccountEmail();
...
function makeSession(key) {
  const S = {
    ...
    graceTimer: null, buffer: [], evictWhenIdle: false,
...
wss.on('connection', (ws) => {
  ...
  function detach(sock, sess) {
    ...
    console.log('[Bridge] Room', sess.key, 'empty — grace period started');
